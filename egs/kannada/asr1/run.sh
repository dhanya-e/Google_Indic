...
# data
datadir=/home/dhanya/PhD/Datasets/Google_Indic/kn/
...
if [ ${stage} -le -1 ] && [ ${stop_stage} -ge -1 ]; then
    ...
    python local/data_prep.py ${datadir} data/local/data/
    local/ka_data_prep.sh ${datadir} ${trans_type} || exit 1
fi

if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    local/ka_format_data.sh
fi
